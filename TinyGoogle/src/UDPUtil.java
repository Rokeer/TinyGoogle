...
public class UDPUtil {
    ...
    public static String receive(int port, int remotePort, String remoteIP)throws Exception{
        ...
        DatagramPacket dp_send= new DatagramPacket(str_send.getBytes(),str_send.length(),InetAddress.getByName(remoteIP),remotePort);
    ...
    public static void send (String str_send, int port) throws IOException{ 
        ...
        while(f){
            ...
            DatagramPacket dp_send= new DatagramPacket(str_send.getBytes(),str_send.length(),dp_receive.getAddress(),remotePort);  
            ds.send(dp_send);  
            dp_receive.setLength(LENGTH); 
            f = false;
        }  
